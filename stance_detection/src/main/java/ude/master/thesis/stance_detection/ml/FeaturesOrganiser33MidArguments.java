...
import ude.master.thesis.stance_detection.processor.NegationFeaturesGenerator33MidArguments;
import ude.master.thesis.stance_detection.processor.RelatedUnrelatedFeatureGenerator33MidArguments;
import ude.master.thesis.stance_detection.processor.SVOFeaturesGenerator33MidArguments;
import ude.master.thesis.stance_detection.processor.SimilarityFeatures33MidArguments;
import ude.master.thesis.stance_detection.processor.StanfordDependencyParser33MidArguments;
import ude.master.thesis.stance_detection.processor.Word2VecDataGenerator33MidArguments;
...
import ude.master.thesis.stance_detection.util.PPDBProcessor33MidArguments;
...
public class FeaturesOrganiser33MidArguments {
	final static Logger logger = Logger.getLogger(FeaturesOrganiser33MidArguments.class);
	// Data
	...
	private FileHashMap<String, int[]> trainArgNeg;
	private FileHashMap<String, int[]> testArgNeg;
	...
	private void initFeat(boolean saveARFF) throws FileNotFoundException, ObjectExistsException, ClassNotFoundException,
		...
		if (useRootDistFeature) {
			trainRootDist = StanfordDependencyParser33MidArguments
					.loadRootDistFeaturesAsHashFile(ProjectPaths.ROOT_DIST_FEATURE_PARTS_ARG33_TRAIN);
			testRootDist = StanfordDependencyParser33MidArguments
					.loadRootDistFeaturesAsHashFile(ProjectPaths.ROOT_DIST_FEATURE_PARTS_ARG33_TEST);

		}

		if (useSVOFeature) {
			trainSVO = SVOFeaturesGenerator33MidArguments.loadSVOFeaturesAsHashFile(ProjectPaths.SVO_PARTS_ARG33_FEATURE_TRAIN);
			testSVO = SVOFeaturesGenerator33MidArguments.loadSVOFeaturesAsHashFile(ProjectPaths.SVO_PARTS_ARG33_FEATURE_TEST);
		}

		if (usePPDBFeature) {
			trainPPDB = PPDBProcessor33MidArguments.loadPPDBFeaturesAsHashFiles(ProjectPaths.PPDB_HUNG_FEATURE_PARTS_ARG33_TRAIN);
			testPPDB = PPDBProcessor33MidArguments.loadPPDBFeaturesAsHashFiles(ProjectPaths.PPDB_HUNG_FEATURE_PARTS_ARG33_TEST);

		}

		if (useNegFeature) {
			trainNeg = NegationFeaturesGenerator33MidArguments.loadNegFeaturesAsHashFile(ProjectPaths.NEG_PARTS_ARG33_FEATURE_TRAIN);
			testNeg = NegationFeaturesGenerator33MidArguments.loadNegFeaturesAsHashFile(ProjectPaths.NEG_PARTS_ARG33_FEATURE_TEST);
		}

		if (useWord2VecAddSimilarity) {
			trainW2VSim = Word2VecDataGenerator33MidArguments.loadWord2VecFeaturesAsHashFile(ProjectPaths.TRAIN_W2V_SUM_SIM_PARTS_ARG33);
			testW2VSim = Word2VecDataGenerator33MidArguments.loadWord2VecFeaturesAsHashFile(ProjectPaths.TEST_W2V_SUM_SIM_PARTS_ARG33);

		}

		if (useOverlapFeature) {
			trainWordOverlap = RelatedUnrelatedFeatureGenerator33MidArguments
					.loadWordsOverlapsFeaturesAsHashFile(ProjectPaths.TRAIN_WORD_OVERLAPS_PARTS_ARG33_PATH);
			testWordOverlap = RelatedUnrelatedFeatureGenerator33MidArguments
					.loadWordsOverlapsFeaturesAsHashFile(ProjectPaths.TEST_WORD_OVERLAPS_PARTS_ARG33_PATH);
		}

		if (useBinaryCooccurraneStopFeatures) {
			trainCoOccuranceStop = RelatedUnrelatedFeatureGenerator33MidArguments
					.loadCoOccStopFeaturesAsHashFile(ProjectPaths.TRAIN_COOCC_PARTS_ARG33_PATH);
			testCoOccuranceStop = RelatedUnrelatedFeatureGenerator33MidArguments
					.loadCoOccStopFeaturesAsHashFile(ProjectPaths.TEST_COOCC_PARTS_ARG33_PATH);
		}

		if (useCharGramsFeatures) {
			trainCgram = RelatedUnrelatedFeatureGenerator33MidArguments
					.loadCharGramsFeaturesAsHashFile(ProjectPaths.TRAIN_CGRAMS_PARTS_ARG33_PATH);
			testCgram = RelatedUnrelatedFeatureGenerator33MidArguments.loadCharGramsFeaturesAsHashFile(ProjectPaths.TEST_CGRAMS_PARTS_ARG33_PATH);
		}

		if (useWordGramsFeatures) {
			trainNgram = RelatedUnrelatedFeatureGenerator33MidArguments.loadNGramsFeaturesAsHashFile(ProjectPaths.TRAIN_NGRAMS_PARTS_ARG33_PATH);
			testNgram = RelatedUnrelatedFeatureGenerator33MidArguments.loadNGramsFeaturesAsHashFile(ProjectPaths.TEST_NGRAMS_PARTS_ARG33_PATH);
		}

		if (useMetricCosineSimilarity) {
			trainStrMetricCosSim = SimilarityFeatures33MidArguments
					.loadCosSimFeaturesAsHashFile(ProjectPaths.TRAIN_COS_SIM_STRMET_PARTS_ARG33_PATH);
			testStrMetricCosSim = SimilarityFeatures33MidArguments
					.loadCosSimFeaturesAsHashFile(ProjectPaths.TEST_COS_SIM_STRMET_PARTS_ARG33_PATH);
		}
		...
		if (useleskOverlap) {
			 ...
			 */
			trainLeskOverlap = SimilarityFeatures33MidArguments.loadLeskFeaturesAsHashFile(ProjectPaths.TRAIN_LESK_PARTS_ARG33_PATH);
			testLeskOverlap = SimilarityFeatures33MidArguments.loadLeskFeaturesAsHashFile(ProjectPaths.TEST_LESK_PARTS_ARG33_PATH);
		}
	...
	public void loadData() throws IOException {
		...
		trainingStances = removeInstances(trainingStances, trainingSummIdBoyMap);
	...
	private List<List<String>> removeInstances(List<List<String>> stances, HashMap<Integer, Map<Integer, String>> idBoyMap) {
		List<List<String>> newStances = new ArrayList<>();
		
		for(List<String> s : stances){
			Integer bodyId = Integer.valueOf(s.get(1));
			if (idBoyMap.containsKey(bodyId))
				newStances.add(s);
		}
